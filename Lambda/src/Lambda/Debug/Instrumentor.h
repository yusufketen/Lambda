	...
	public:
		...
		void BeginSession(const std::string& name, const std::string& filepath = "results.json")
		{
			...
			if (m_CurrentSession)
			{
				...
				{
					LM_CORE_ERROR("Instrumentor::BeginSession('{0}') when session '{1}' already open.", name, m_CurrentSession->Name);
				}
				InternalEndSession();
			}
...
#define LM_PROFILE_BEGIN_SESSION(name, filepath) ::Lambda::Instrumentor::Get().BeginSession(name, filepath)
#define LM_PROFILE_END_SESSION() ::Lambda::Instrumentor::Get().EndSession()
#define LM_PROFILE_SCOPE_LINE2(name, line) constexpr auto fixedName##line = ::Lambda::InstrumentorUtils::CleanupOutputString(name, "__cdecl ");\
											   ::Lambda::InstrumentationTimer timer##line(fixedName##line.Data)
